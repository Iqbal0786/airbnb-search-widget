      ...
      >
       {
        tabItems.map((item, index) =>{
          return (
            <Tab  label={<span style={{textTransform:'capitalize'}}>{item.name}</span>} icon={<img src={`${item.image}`} style={{color:'black' , width:'24px',height:'24px' }}/>}  sx={{fontSize:'12px'}}/>
          )
        })
       }
